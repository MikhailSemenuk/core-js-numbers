...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const denominator1 = (x1 + y1 ** 2) ** 0.5;
  const denominator2 = (x2 ** 2 + y2 ** 2) ** 0.5;
  const c = numerator / (denominator1 * denominator2);
  return Math.acos(c);
}
...
function roundToPowerOfTen(num, pow) {
  const rate10 = 10 ** pow;
  return Math.round(num / rate10) * rate10;
}
 ...
 */
function isPrime(n) {
  for (let denominator = 2; denominator < n; denominator += 1) {
...
function getFibonacciNumber(index) {
  ...
  const notRoundedAnswer =
    (goldenRatio ** index - (1 - goldenRatio) ** index) / 5 ** 0.5;
  return Math.round(notRoundedAnswer);
}
...
function isPowerOfTwo(num) {
  for (let checkValue = 1; checkValue <= num; checkValue *= 2) {
    if (checkValue === num) {
      return true;
    }
  }
...
function getSumOfNumbers(x1, x2, x3) {
  return [x1, x2, x3].reduce((a, b) => a + b, 0).toFixed(1);
}
